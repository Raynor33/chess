...
class ChessControllerSpec extends PlaySpec with Results with MockitoSugar with BeforeAndAfter {
  ...
  "A ChessController" should {
    "start a game" in {
      ...
      chess.model.displayGame.reads(Json.parse(contentAsString(result))).get mustBe DisplayGame(id, game)
    ...
    "give the correct response for the first successful move" in {
      ...
      val result: Future[Result] = controller.move(id)(FakeRequest("POST", "/games/" + id + "/moves").withBody(
        chess.model.moveInstructionFormats.writes(instruction))
      )
      chess.model.displayMove.reads(Json.parse(contentAsString(result))).get mustBe DisplayMove(id, instruction)
      header("Location", result) mustBe Some("/games/id/moves/0")
      status(result) mustBe 201
      verify(gameService, times(1)).doMove(id, instruction)
    }
    "give the correct response for the second successful move" in {
      val instruction = StandardMoveInstruction(Square(1,2), Square(3,4))
      ...
      val result: Future[Result] = controller.move(id)(FakeRequest("POST", "/games/" + id + "/moves").withBody(
        chess.model.moveInstructionFormats.writes(instruction1))
      )
      chess.model.displayMove.reads(Json.parse(contentAsString(result))).get mustBe DisplayMove(id, instruction1)
